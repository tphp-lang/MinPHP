...
php main.php run examples/05_package.php     # 包管理示例（#import 引入 ext/hello）
...
php tests/run.php                            # 跑测试（68 个用例，含多文件/内存/推断/phpc/可见性/闭包/heredoc/枚举/析构/指令安全/平台条件/map/trait/abstract/instanceof/运行时 panic/返回路径内存安全/包导入）
php tests/shared.php                         # 库模式测试（shared 命令 + #[export] 符号导出）
php tests/dot.php                            # `.` 指令测试（递归展开 + 排除规则）
php tests/cross.php                          # 交叉编译测试（4 个目标）
php tests/packages.php                       # 包管理测试（#import 装配/传递依赖/环依赖/C 能力/自带包/来源唯一性）
```
